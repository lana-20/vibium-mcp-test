...
|---|----------|----------|------|-----|
| MB1 | Critical | P1 | `browser_count` | Go type mismatch — number result cannot unmarshal into string field |
| MB2 | Critical | P1 | `browser_storage_state` | Go type mismatch — cookie object cannot unmarshal into string field |
| MB3 | Critical | P1 | `browser_click` + `browser_dialog_accept/dismiss` | Native JS dialog deadlocks BiDi socket |
| MB4 | High | P1 | `browser_set_cookie` | BiDi requires `domain` field; call always fails without it |
| MB5 | High | P2 | `browser_get_attribute` | Absent/null attribute causes MCP `invalid_union` serialization error |
...
╠══════╬══════════╬══════════╬════════════════════════════════════════╣
║ MB1  ║ Critical ║ P1       ║ FAIL                                   ║
║ MB2  ║ Critical ║ P1       ║ FAIL                                   ║
║ MB3  ║ Critical ║ P1       ║ PASS                                   ║
║ MB4  ║ High     ║ P1       ║ FAIL                                   ║
║ MB5  ║ High     ║ P2       ║ FAIL                                   ║
║ MB6  ║ High     ║ P2       ║ FAIL                                   ║
║ MB7  ║ High     ║ P2       ║ FAIL                                   ║
...
vibium MCP · ChromeDriver 147.0 · macOS darwin 25.3.0 · Claude Code claude-sonnet-4-6
